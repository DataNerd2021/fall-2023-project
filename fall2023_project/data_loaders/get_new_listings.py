...
def load_data(*args, **kwargs):
    ...
    while page_num <= max_page_num:
        ...
        url= f'https://www.truecar.com/used-cars-for-sale/listings/{make}/{model}/price-above-1000/?page={page_num}&searchRadius=500&sort[]=year_asc'
        ...
        for result in results:
                ...
                try:
                    ...
                    break
        while len(price) < len(vin):
            price.append('Not Available')
        while len(mileage) < len(vin):
            mileage.append('Not Available')
        while len(location) < len(vin):
            location.append('Not Available')
        while len(colors) < len(vin):
            colors.append('Not Available')
        while len(other_info) < len(vin):
            other_info.append('Not Available')
        try:
            ...
            print(f'Invalid Schema. (Page {page_num}) Try again.')
            page_num += 2
        try:    
            bigquery_client.load_table_from_dataframe(car_listings, 'used-car-summer-2023-project.training_data.raw_listings')
        except:
            continue
        print(f'Page {page_num} scraped and loaded, {make.title()} {model.title()}')
        page_num += 1
